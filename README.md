# 삼목기업 AI 훈련로드맵 · 기업소개 홈페이지 초안

## 최종 전달 링크

- 삼목기업 소개 홈페이지: https://kclock-boop.github.io/sammok/
- 선박의장 안전활동지수 대시보드: https://kclock-boop.github.io/sammok/docs/SAMMOK_ship_outfitting_safety_dashboard.html

삼목기업의 2026년 8월 28일(금) 예정 `AI 훈련로드맵 2차 컨설팅`과,
조선·해양 협력사 소개형 홈페이지 검토를 위해 정리한 저장소입니다.

이 저장소는 지난 `snd` 저장소처럼, 단순 파일 보관이 아니라 아래 흐름이 한눈에 보이도록 구성했습니다.

- 2차 컨설팅에서 바로 설명할 수 있는 소개형 허브 페이지
- 한화오션 협력사 맥락을 참고한 삼목기업 기업소개 홈페이지 초안
- 선박의장 안전활동지수 Dashboard 시안
- 컨설팅 진행용 구성요약, 발표 멘트, 회의록 초안
- 후속 발표자료 업로드를 위한 정리된 폴더 구조

## 바로가기

- 컨설팅 허브 페이지: [site/index.html](./site/index.html)
- GitHub Pages 메인 (PC/휴대폰 공통 링크): [https://kclock-boop.github.io/sammok/](https://kclock-boop.github.io/sammok/)
- 허브페이지 직접열기 (PC/휴대폰 공통 링크): [https://kclock-boop.github.io/sammok/site/index.html](https://kclock-boop.github.io/sammok/site/index.html)
- 1차 컨설팅 HTML 시안 1 직접열기: [https://kclock-boop.github.io/sammok/docs/1st-consulting/index.html](https://kclock-boop.github.io/sammok/docs/1st-consulting/index.html)
- 1차 컨설팅 HTML 시안 2 직접열기: [https://kclock-boop.github.io/sammok/docs/1st-consulting/index1.html](https://kclock-boop.github.io/sammok/docs/1st-consulting/index1.html)
- 2차 대시보드 직접열기 (PC/휴대폰 공통 링크): [https://kclock-boop.github.io/sammok/docs/SAMMOK_ship_outfitting_safety_dashboard.html](https://kclock-boop.github.io/sammok/docs/SAMMOK_ship_outfitting_safety_dashboard.html)
- 선박의장 안전활동지수 Dashboard: [docs/SAMMOK_ship_outfitting_safety_dashboard.html](./docs/SAMMOK_ship_outfitting_safety_dashboard.html)
- 1차 컨설팅 HTML 시안 1: [docs/1st-consulting/index.html](./docs/1st-consulting/index.html)
- 1차 컨설팅 HTML 시안 2: [docs/1st-consulting/index1.html](./docs/1st-consulting/index1.html)
- 2차 컨설팅 구성요약: [proposal/삼목기업_AI훈련로드맵_2차컨설팅_구성요약.md](./proposal/%EC%82%BC%EB%AA%A9%EA%B8%B0%EC%97%85_AI%ED%9B%88%EB%A0%A8%EB%A1%9C%EB%93%9C%EB%A7%B5_2%EC%B0%A8%EC%BB%A8%EC%84%A4%ED%8C%85_%EA%B5%AC%EC%84%B1%EC%9A%94%EC%95%BD.md)
- 발표 멘트 / Q&A: [proposal/삼목기업_AI훈련로드맵_2차컨설팅_발표멘트_QA.md](./proposal/%EC%82%BC%EB%AA%A9%EA%B8%B0%EC%97%85_AI%ED%9B%88%EB%A0%A8%EB%A1%9C%EB%93%9C%EB%A7%B5_2%EC%B0%A8%EC%BB%A8%EC%84%A4%ED%8C%85_%EB%B0%9C%ED%91%9C%EB%A9%98%ED%8A%B8_QA.md)
- 회의록 반완성본: [proposal/삼목기업_AI훈련로드맵_2차컨설팅_회의록_반완성본.md](./proposal/%EC%82%BC%EB%AA%A9%EA%B8%B0%EC%97%85_AI%ED%9B%88%EB%A0%A8%EB%A1%9C%EB%93%9C%EB%A7%B5_2%EC%B0%A8%EC%BB%A8%EC%84%A4%ED%8C%85_%ED%9A%8C%EC%9D%98%EB%A1%9D_%EB%B0%98%EC%99%84%EC%84%B1%EB%B3%B8.md)
- 공개 정보 조사 메모: [proposal/삼목기업_공개정보_조사메모.md](./proposal/%EC%82%BC%EB%AA%A9%EA%B8%B0%EC%97%85_%EA%B3%B5%EA%B0%9C%EC%A0%95%EB%B3%B4_%EC%A1%B0%EC%82%AC%EB%A9%94%EB%AA%A8.md)

## 이번 저장소의 목적

- 2차 컨설팅에서 `AI 훈련로드맵`을 설명할 때 논의 축을 명확히 정리
- 삼목기업을 조선·해양 협력사 관점에서 소개할 수 있는 홈페이지 초안 마련
- 선박의장 현장 특성에 맞는 `안전활동지수 Dashboard` 시안을 제시
- 경영진, 센터팀장, 실무자가 같은 자료를 보고 다른 질문을 할 수 있게 구조화
- 후속 PoC, 교육, 현장 코칭, 보고체계 자동화 논의를 이어갈 수 있는 기준 문서 확보

## 핵심 산출물

### 1) 컨설팅 허브 페이지

- 파일: [site/index.html](./site/index.html)
- 설명: 한화오션 계열 조선·해양 홈페이지의 톤을 참고해, 삼목기업을 협력사 관점에서 소개할 수 있도록 정리한 기업소개형 허브 페이지

### 2) 선박의장 안전활동지수 Dashboard

- 파일: [docs/SAMMOK_ship_outfitting_safety_dashboard.html](./docs/SAMMOK_ship_outfitting_safety_dashboard.html)
- 설명: 고소작업, 중량물 취급, 화기작업, 통로관리 등 선박의장 공정의 안전 리스크를 중심으로 구성한 단일 HTML Dashboard

### 3) 2차 컨설팅 구성요약

- 파일: [proposal/삼목기업_AI훈련로드맵_2차컨설팅_구성요약.md](./proposal/%EC%82%BC%EB%AA%A9%EA%B8%B0%EC%97%85_AI%ED%9B%88%EB%A0%A8%EB%A1%9C%EB%93%9C%EB%A7%B5_2%EC%B0%A8%EC%BB%A8%EC%84%A4%ED%8C%85_%EA%B5%AC%EC%84%B1%EC%9A%94%EC%95%BD.md)
- 설명: 세션 목적, 논의 순서, 훈련 우선순위, Dashboard 활용 방식, 후속 액션을 정리한 실무용 문서

### 4) 발표 멘트 / 예상 Q&A

- 파일: [proposal/삼목기업_AI훈련로드맵_2차컨설팅_발표멘트_QA.md](./proposal/%EC%82%BC%EB%AA%A9%EA%B8%B0%EC%97%85_AI%ED%9B%88%EB%A0%A8%EB%A1%9C%EB%93%9C%EB%A7%B5_2%EC%B0%A8%EC%BB%A8%EC%84%A4%ED%8C%85_%EB%B0%9C%ED%91%9C%EB%A9%98%ED%8A%B8_QA.md)
- 설명: 3분 발표문, 1분 요약문, 질문 대응 포인트를 정리한 문서

### 5) 회의록 반완성본

- 파일: [proposal/삼목기업_AI훈련로드맵_2차컨설팅_회의록_반완성본.md](./proposal/%EC%82%BC%EB%AA%A9%EA%B8%B0%EC%97%85_AI%ED%9B%88%EB%A0%A8%EB%A1%9C%EB%93%9C%EB%A7%B5_2%EC%B0%A8%EC%BB%A8%EC%84%A4%ED%8C%85_%ED%9A%8C%EC%9D%98%EB%A1%9D_%EB%B0%98%EC%99%84%EC%84%B1%EB%B3%B8.md)
- 설명: 회의 당일 참석자, 결정사항, 자료요청, 기한만 채우면 바로 마무리할 수 있도록 미리 구성한 문서

### 6) 공개 정보 조사 메모

- 파일: [proposal/삼목기업_공개정보_조사메모.md](./proposal/%EC%82%BC%EB%AA%A9%EA%B8%B0%EC%97%85_%EA%B3%B5%EA%B0%9C%EC%A0%95%EB%B3%B4_%EC%A1%B0%EC%82%AC%EB%A9%94%EB%AA%A8.md)
- 설명: 공개 채용/기업정보 기준으로 확인한 범위와, 현장 확인이 필요한 항목을 분리한 메모

## 폴더 구조

- `site/`: 컨설팅 허브 페이지
- `docs/`: Dashboard 및 공유용 웹 문서
- `proposal/`: 컨설팅 기획, 발표, 회의 문서
- `ppt/`: 발표자료 업로드용 폴더
- `.github/workflows/`: GitHub Pages 배포용 워크플로

## 활용 시나리오

- 2차 컨설팅 시작 전에 팀장/실무자와 사전 공유
- 회의 중 Dashboard 시안과 로드맵 동시 설명
- 회사소개 초안으로 대외 공유 시 첫 인상용 페이지로 활용
- 회의 후 Action Item과 후속 구축 범위 정리
- GitHub 저장소를 기반으로 Pages 또는 링크 공유 확장

## 공유용 문구

### 1) 담당자에게 보내는 기본 문구

안녕하세요. 검토용 링크 전달드립니다.  
삼목기업 소개 페이지(PC/휴대폰 공통): https://kclock-boop.github.io/sammok/  
안전활동지수 대시보드(PC/휴대폰 공통): https://kclock-boop.github.io/sammok/docs/SAMMOK_ship_outfitting_safety_dashboard.html

### 2) 짧은 1줄 문구

삼목기업 검토용 링크 전달드립니다. 소개 페이지(PC/휴대폰 공통): https://kclock-boop.github.io/sammok/ / 대시보드(PC/휴대폰 공통): https://kclock-boop.github.io/sammok/docs/SAMMOK_ship_outfitting_safety_dashboard.html

### 3) 조금 더 부드러운 문구

안녕하세요. 2026년 8월 28일(금) 예정 컨설팅 검토용 링크 먼저 공유드립니다.  
- 소개 페이지(PC/휴대폰 공통): https://kclock-boop.github.io/sammok/  
- 대시보드(PC/휴대폰 공통): https://kclock-boop.github.io/sammok/docs/SAMMOK_ship_outfitting_safety_dashboard.html

## 비고

- 본 저장소의 일부 수치와 Dashboard 데이터는 현장 논의를 위한 예시값입니다.
- 회사 상세 정보 중 공개 자료로만 확인된 내용은 별도 조사 메모에 분리했습니다.
- 회사연혁, 사업영역, 문의 정보 등은 공식 자료 확인 후 실제 정보로 교체하는 것을 권장합니다.
- PPT 원본은 아직 작업공간에 없어서 폴더 구조만 먼저 준비했습니다.
